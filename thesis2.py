...
data1 = pd.read_csv('Domain-I_load_solution_UPS.txt')
data2 = pd.read_csv('testCourantReseauSortieUps1.txt')
...
data2['SMA'] = (data2['I_load_rms'].rolling(1).max()).rolling(5).min()
...
plt.plot(data2['Domain'],data2['I_load_rms'], label='Experimental current', marker='x', markerSize = 2)

# Tracer le graphe pour data
plt.plot(data1['Domain'],data1['I_load'], label='Simulation current', marker='o', markerSize = 2,linestyle='-')
...
plt.title('Graph of simulation and experimental current with UPS solution')
...
fileNameCurrent = 'Current_With_UPS_NoOutliers' +  get_datetime() + '.png'
